...
public class LoginController implements CommunityConstant {
    ...
    public String register(Model model, User user){
        ...
        if(map==null||map.isEmpty()){
            model.addAttribute("msg","注册成功，我们已经向您的邮箱发送了一封激活邮件，请查收！");
...
//        session.setAttribute("kaptcha",text);
        ...
        cookie.setMaxAge(60);   //验证码60秒过期
        cookie.setPath(contextPath);       //整个项目路径都有效
        response.addCookie(cookie);
        //将验证码存入Redis，且设置过期时间为60s
...
//        String kaptcha = (String) session.getAttribute("kaptcha");
        ...
        //检查账号密码
        int expiredSeconds = rememberme ? REMEMBER_EXPIRED_SECONS : DEFAULT_EXPIRED_SECONDS;
